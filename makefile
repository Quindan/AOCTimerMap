...
help:
	...
	@echo ""
	@echo "Production Build Commands (NEW - Recommended):"
	@echo "  prod-build       Build Docker image with production approach"
	@echo "  prod-up          Start production services"
	@echo "  prod-down        Stop production services"
	@echo "  prod-logs-new    Follow production container logs"
	@echo "  prod-test        Test the production build locally"
...
prod-build:
	@echo "🏗️  Building Docker image with production approach..."
...
prod-up:
	...
	docker-compose -f docker-compose.production.yml up -d aoctimermap-main
	@echo "✅ Production services started successfully!"
	@echo "🌐 Main application: http://localhost/"
	@echo "📊 Check status with: make prod-logs-new"
...
prod-logs-new:
	@echo "📋 Following production container logs (Ctrl+C to quit)..."
	docker-compose -f docker-compose.production.yml logs -f aoctimermap-main
...
prod-test:
	...
	@if curl -s -o /dev/null -w "%{http_code}" http://localhost/ | grep -q "200"; then \
		...
		echo "❌ Landing page is not accessible. Check logs with: make prod-logs-new"; \
	fi

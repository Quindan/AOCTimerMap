...
function handleGetAllNamedMobs($db) {
    ...
    $sql = "SELECT * FROM named_mobs WHERE 1=1";
    $params = [];
...
function handleSearchNamedMobs($db) {
    ...
    $sql = "SELECT * FROM named_mobs WHERE name LIKE ? OR slug LIKE ?";
    $params = ["%$query%", "%$query%"];
    ...
    $sql .= " ORDER BY level, name LIMIT 20";
...
function handleGetNamedMob($db, $mobId) {
    $stmt = $db->prepare("SELECT * FROM named_mobs WHERE id = ? OR slug = ?");
